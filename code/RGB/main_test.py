...
parser.add_argument('--dataset_name', default='CBSD68', type=str)
...
if __name__ == '__main__':
    ...
    save_path = os.path.join(args.save_dir, 'results', 'DUMRN_' + args.dataset_name + '_sigma' + str(int(args.sigma)))
    ...
    model = torch.load(os.path.join(model_path, 'RGB_sigma' + str(int(args.sigma)) + '.pth'))
    model.eval()
    if cuda:
        model = model.cuda()
